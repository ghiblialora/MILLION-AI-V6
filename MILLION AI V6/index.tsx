                            ...
                            {categories.map(cat => (
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-full text-sm flex items-center gap-2 transition-all ${
                                        category === cat.id ? styles.modalChipActive : styles.modalChip
                                    }`}
                                >
                                    <span>{cat.icon}</span>
                                    <span>{cat.label}</span>
...
const SettingsModal = ({ 
  ...
  useEffect(() => {
    if (isOpen && user) setEmail(user.email || '');
  }, [isOpen, user]);
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex items-center justify-center ${styles.modalOverlay}`}>
      <div className={`w-full max-w-md p-6 relative animate-fade-in-up ${styles.modalContent}`}>
        <div className="flex justify-between items-center mb-6">
            <h2 className={`text-xl font-bold ${theme === 'hacker' ? 'tracking-widest' : 'tracking-tight'}`}>
                {theme === 'hacker' ? 'SYSTEM CONFIG' : 'Settings'}
            </h2>
            <button onClick={onClose} className="opacity-50 hover:opacity-100 text-lg">✕</button>
        </div>
        ...
        <form onSubmit={handleUpdate} className="space-y-4">
            <div className="space-y-1">
                <label className={`text-xs uppercase font-semibold tracking-wider ${theme === 'hacker' ? 'opacity-70' : 'text-gray-500'}`}>Email</label>
                <input 
                    type="email" 
                    value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    className={`w-full px-4 py-2 outline-none transition-all ${styles.modalInput}`}
                />
            </div>
            <div className="space-y-1">
                <label className={`text-xs uppercase font-semibold tracking-wider ${theme === 'hacker' ? 'opacity-70' : 'text-gray-500'}`}>New Password</label>
                <input 
                    type="password" 
                    value={password}
            ...
            <div className={`p-4 rounded-lg ${theme === 'hacker' ? 'bg-[#001a00] border border-[#003b00]' : 'bg-gray-50 border border-gray-100'}`}>
                <div className="space-y-1">
                    <label className={`text-xs uppercase font-bold tracking-wider ${theme === 'hacker' ? 'text-[#00ff41]' : 'text-gray-700'}`}>Current Password (Required)</label>
                    <input 
                        type="password" 
            ...
            <div className="flex justify-end gap-3 pt-2">
                <button type="button" onClick={onClose} className={`px-4 py-2 font-medium ${styles.modalBtnSecondary}`}>Cancel</button>
                <button type="submit" disabled={loading} className={`px-6 py-2 font-medium shadow-md ${styles.modalBtnPrimary}`}>
                    {loading ? 'UPDATING...' : (theme === 'hacker' ? 'EXECUTE' : 'Save Changes')}
                </button>
            </div>
        </form>
      </div>
        ...
        <div className={`fixed inset-0 z-[100] flex items-center justify-center ${styles.modalOverlay}`}>
            <div className={`w-full max-w-md p-6 relative animate-fade-in-up ${styles.modalContent}`}>
                <div className="flex justify-between items-center mb-4">
                    <h2 className={`text-xl font-bold ${theme === 'hacker' ? 'tracking-widest' : 'tracking-tight'}`}>
                        {theme === 'hacker' ? 'Gnenerating LINK...' : 'Share Chat'}
                    </h2>
                    <button onClick={onClose} className="opacity-50 hover:opacity-100 text-lg">✕</button>
                </div>
                <p className={`text-sm mb-4 ${theme === 'hacker' ? 'text-[#008f11]' : 'text-gray-500'}`}>
                ...
                <div className="flex gap-2">
                    <input 
                        readOnly 
                        value={link} 
                        className={`flex-1 px-4 py-2 text-sm outline-none ${styles.modalInput} opacity-70`} 
                    />
                    <button onClick={handleCopy} className={`px-4 py-2 font-medium shadow-sm ${styles.modalBtnPrimary}`}>
                        {copied ? 'Copied!' : 'Copy'}
                    </button>
                </div>
            </div>
...
const Sidebar = ({ 
  ...
  onDeleteProject: (e: React.MouseEvent, id: string) => void, // New prop type
  onLogout: () => void,
  onOpenSettings: () => void
}) => {
  const styles = getThemeStyles(theme);
      ...
      `}>
                 ...
                 {projects.map(proj => (
                     <div key={proj.id} className="relative group">
                         <button
                            ...
                            className={`w-full text-left px-4 py-2 text-sm flex items-center gap-3 transition-all pr-8 ${activeProjectId === proj.id ? styles.sidebarActive : styles.sidebarText}`}
                         >
                            <span>{proj.icon}</span>
                            <span className="truncate">{proj.name}</span>
                         </button>
                         <button
                             onClick={(e) => onDeleteProject(e, proj.id)}
           ...
           {filteredSessions.map((session) => (
             <div key={session.id} className="relative group">
               <button 
                 ...
                 className={`w-full text-left px-4 py-3 text-sm cursor-pointer truncate transition-all pr-8 ${
        ...
        {theme === 'hacker' && (
          <div className="p-4 border-t border-[#003b00] text-[10px] flex justify-between items-center text-[#008f11]">
             ...
             <div className="flex gap-2">
                <button onClick={onOpenSettings} className="hover:text-[#00ff41] transition-colors"><IconSettings /></button>
          ...
          <div className="p-4 border-t border-gray-100">
              <div className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-100 transition-colors group">
                   ...
                   <div className="flex items-center gap-1">
                       <button onClick={onOpenSettings} className="text-gray-400 hover:text-black p-2 rounded-md hover:bg-gray-200 transition-all" title="Settings">
                          <IconSettings />
                       </button>
...
const App = () => {
  ...
  const handleDeleteProject = (e: React.MouseEvent, id: string) => {
      e.stopPropagation();
      const confirmMsg = theme === 'hacker' ? 'EXECUTE DELETE_PROJECT?' : 'Delete this project? Chats will be moved to All Chats.';
      if (window.confirm(confirmMsg)) {
          ...
          // Unassign chats
          setSessions(prev => {
              const updated = prev.map(s => s.projectId === id ? { ...s, projectId: undefined } : s);
              localStorage.setItem('million_ai_sessions', JSON.stringify(updated));
              return updated;
          });
  ...
  const handleShareChat = () => {
      if (messages.length === 0) {
          alert("Cannot share an empty conversation.");
          return;
      }
      // Simple sharing via URL params (limitations apply to length)
      try {
          ...
          if (url.length > 8000) {
               alert("Conversation is too long to share via link.");
               return;
          }
           ...
           <div className="max-w-[850px] mx-auto">
               {attachments.length > 0 && (
                 <div className="flex flex-wrap gap-2 mb-3 pl-2 animate-in slide-in-from-bottom-2 fade-in">
                   {attachments.map(att => (
                     <div key={att.id} className={`relative flex items-center gap-2 pl-2 pr-8 py-1.5 rounded-lg text-xs font-medium ${styles.chip} overflow-hidden`}>
                        {att.isLoading && (
