...
const IconMillion = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" className="w-full h-full">
    {/* Bold M with wings effect */}
    <path d="M3 20 L3 8 L6 8 L9 14 L12 8 L15 14 L18 8 L21 8 L21 20 L18 20 L18 12 L15 18 L12 12 L9 18 L6 12 L6 20 Z" opacity="0.9"/>
    
    {/* Wing accents */}
    <path d="M2 10 L1 8 L2 9 L3 7 L3 9 Z" opacity="0.6"/>
    <path d="M22 10 L23 8 L22 9 L21 7 L21 9 Z" opacity="0.6"/>
    
    {/* Top accent */}
    <path d="M10 6 L12 4 L14 6" stroke="currentColor" strokeWidth="1" fill="none" opacity="0.5"/>
  </svg>
);
...
const App = () => {
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    // Check API key availability (from localStorage or env)
    const apiKey = localStorage.getItem('gemini_api_key') || process.env.API_KEY;
    if (!apiKey) {
      setMessages(prev => [...prev, { id: Date.now().toString(), role: 'model', text: "Error: API Key is missing. Please set the API_KEY in admin panel or environment variable.", timestamp: Date.now() }]);
      return;
    }
    ...
    try {
      ...
      messages.forEach(msg => {
        ...
        if (msg.attachments) {
          msg.attachments.forEach(att => {
            if (att.type === 'image') {
              // Skip images that are URLs (from image generation mode)
              // Only process base64 encoded images
              if (att.data.startsWith('data:') || !att.data.startsWith('http')) {
                const base64Data = att.data.includes(',') ? att.data.split(',')[1] : att.data;
                if (base64Data) {
                  parts.push({ inlineData: { mimeType: att.mimeType, data: base64Data } });
                }
              }
              // For URL images, just add a text description
              else {
                parts.push({ text: `[Generated Image: ${att.name}]` });
              }
            } else {
              parts.push({ text: `\n\nFile: ${att.name}\n${att.data}\n\n` });
            }
      ...
      for (const att of currentAttachments) {
        if (att.type === 'code' || att.type === 'text') {
          ...
          // Only process base64 images, skip URL images
          if (att.data.startsWith('data:') || !att.data.startsWith('http')) {
            const base64Data = att.data.includes(',') ? att.data.split(',')[1] : att.data;
            currentParts.push({
    ...
    >
      ...
      <div className={`flex-1 flex flex-col relative min-w-0 ${theme === 'modern' ? 'bg-white' : 'bg-[#050505]'}`}>
        ...
        <main className="flex-1 overflow-y-auto scroll-smooth" ref={chatContainerRef}>
          <div className="max-w-[900px] mx-auto px-4 md:px-6 pb-6 pt-8 flex flex-col gap-6">
            ...
            {messages.map((msg) => (
              <div key={msg.id} className={`flex w-full group gap-3 md:gap-4 animate-fade-in-up ${msg.role === 'user' ? styles.msgUserContainer : styles.msgModelContainer}`}>

                {msg.role === 'model' && (
                  <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${styles.avatarModel}`}>
                ...
                <div className={`relative flex flex-col max-w-full ${msg.role === 'user' ? 'items-end' : 'items-start w-full min-w-0'}`}>
                  ...
                  <div className={`${msg.role === 'user' ? styles.msgUserBubble : styles.msgModelBubble}`}>
                    ...
                    {msg.role === 'user' ? (
                      <span className="inline-block break-words leading-relaxed">{msg.text}</span>
                    ) : (
                ...
                {msg.role === 'user' && (
                  <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-sm font-semibold ${styles.avatarUser}`}>
